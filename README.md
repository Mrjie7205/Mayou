...
株洲碰胡云手机辅助工具 · 个人项目 · 仅用于欢乐豆纯娱乐房间

## 这是什么

一个挂在 PC 上的桌面工具。在电脑上用红手指云手机打开闲逸棋牌·株洲碰胡时，Mayou 实时读取屏幕、追踪牌局、给出出牌建议。我手动按建议出牌。
...
| [docs/07-zhuzhou-pengHu-rules.md](docs/07-zhuzhou-pengHu-rules.md) | 株洲碰胡规则（当前引擎依据） | 调引擎、对胡牌型时 |
| [docs/06-zhuanzhuan-rules.md](docs/06-zhuanzhuan-rules.md) | 转转麻将规则（已 deprecated，参考存档） | 极少 |
| [docs/decisions/](docs/decisions/) | 关键决策记录（ADR） | 回顾"为什么这么定"时 |

## 当前状态

🟢 **Sprint -1 已完成**（2026-05-12）。Sprint 0 待开工。还没写任何代码。

已经定下：
- 平台：红手指云手机 PC 客户端 + 闲逸棋牌（[ADR-001](docs/decisions/ADR-001-platform.md)）
- 核心游戏：株洲碰胡（[ADR-002](docs/decisions/ADR-002-game-switch.md)）

下一步：进入 Sprint 0 脚手架，见 [ROADMAP.md](ROADMAP.md)。
...
- **欢乐场原则**：本项目永不用于现金/积分/比赛场、永不分发、永不开源。
